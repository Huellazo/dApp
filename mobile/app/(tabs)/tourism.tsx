...
export default function TourismTabScreen() {
  ...
  const featuredPOIs = tourismPOIs.slice(0, 3);
  ...
  return (
    <View className="flex-1 bg-background pt-12 px-4 pb-24">
      ...
      <View className="flex-row justify-between items-center mb-6">
        <View>
          <Text className="text-4xl font-black text-border uppercase tracking-tight">{t('tourism.title')}</Text>
          <Text className="text-sm font-bold text-primary">{t('tourism.subtitle')}</Text>
        </View>
        <View className="bg-primary p-3 rounded-full border-4 border-border shadow-brutal-sm">
          <FontAwesome5 name="compass" size={20} color="#FAF9F6" />
        </View>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Text className="text-xl font-black text-border mb-4 uppercase mt-4">{t('tourism.routes')}</Text>
        <View className="mb-8">
          {tourismPOIs.map(poi => (
            <Pressable key={`list-${poi.id}`} onPress={() => router.push(`/tourism/${poi.id}`)} className="active:opacity-80 mb-4">
              <BrutalistCard colorClass="bg-background p-0 w-full overflow-hidden" variant="info">
                <View className="w-full h-48 border-b-4 border-border bg-primary/20 relative justify-center items-center overflow-hidden">
                  {poi.image && (
                  ...
                  <View className="absolute top-2 right-2 bg-accent2 px-2 py-1 border-2 border-border shadow-brutal-sm">
                    <Text className="text-background font-black text-xs uppercase">+{poi.reward} HZ</Text>
                  </View>
                </View>
                  ...
                  <View className="flex-row items-center justify-between mt-2">
                    ...
                    <View className="flex-row items-center">
                      ...
                      {(poi as any).nftReward && (
                        <FontAwesome5 name="award" solid size={18} color={colors.primary} />
                      )}
                    </View>
                  </View>
                </View>
