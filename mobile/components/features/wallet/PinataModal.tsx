...
export function PinataModal({ visible, onClose }: Props) {
  ...
  const handleHit = () => {
    ...
    if (points < 100) {
      alert("Not enough HUELLAZOS to break the piñata!");
      onClose();
      return;
    }
  ...
  // Fake animation with tailwind classes
  const getShakeClass = () => {
      ...
      <View className="flex-1 bg-black/80 justify-center items-center px-4">
        {state === 'idle' && (
          <View className="items-center">
            <Text className="text-white font-black text-3xl uppercase mb-2">Tap 3 Times!</Text>
        ...
        {state === 'broken' && loot && (
          <BrutalistCard colorClass="bg-background w-full max-w-sm p-0 overflow-hidden">
            <View className="bg-accent2 p-4 border-b-4 border-border flex-row justify-between items-center">
               <Text className="text-border font-black text-xl uppercase">PIÑATA BROKEN!</Text>
               <FontAwesome5 name="star" size={24} color={colors.border} />
            </View>
            ...
            <View className="p-4 items-center">
               ...
               <Text className="text-border font-black text-xl uppercase mb-2">You Found:</Text>
               ...
               <View className="w-full">
                 <BrutalistButton title="CLAIM REWARD" colorClass="bg-accent1" onPress={onClose} />
               </View>
            </View>
          </BrutalistCard>
        )}
